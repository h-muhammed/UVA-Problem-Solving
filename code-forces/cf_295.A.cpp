...
TEMPLATE T bigmod(T n, int p, T m)
{
    if(!p) return 1;
    else
    {
...
int main()
{
    int n,m,i, d=0,a,b,k=0;
    char arr[101];
    memset(ar, 0, sizeof(ar));
    scanf("%d %s", &n, &arr);
    for(i=0; i<n; ++i)
    {
        if(arr[i]=='a'||arr[i]=='c'||arr[i]=='b'||arr[i]=='d'||arr[i]=='e'||arr[i]=='f'||arr[i]=='g'||arr[i]=='h'||arr[i]=='i'||
           arr[i]=='j'||arr[i]=='k'||arr[i]=='l'||arr[i]=='m'||arr[i]=='n'||arr[i]=='o'||arr[i]=='p'||arr[i]=='q'||
           arr[i]=='r'||arr[i]=='s'||arr[i]=='t'||arr[i]=='u'||arr[i]=='v'||arr[i]=='w'||arr[i]=='x'||arr[i]=='y'||arr[i]=='z')
           {
        ar[arr[i]-32]=1;
        ar[arr[i]]=1;
           }
